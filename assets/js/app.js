...
const TOPICS_CONFIG = [
    ...
    { id: 9, title: "9. Q&A Dialogues", desc: "Hội thoại cùng Cô giáo Chim Xanh", icon: "💬", color: "cyan" },
...
const GREETINGS_STUDENT = [
    "Chào {name}, cô Chim Xanh rất vui được học tiếng Anh cùng con hôm nay!",
    "Chào mừng {name} quay lại! Sẵn sàng chinh phục thêm thật nhiều từ vựng mới chưa nào?",
    "Cô Chim Xanh chào {name}! Cùng nhau nói tiếng Anh thật giỏi hôm nay nhé!",
    "Chào con yêu {name}, hôm nay chúng mình cùng khám phá thế giới tiếng Anh nhé!",
    "Chào mừng {name} đến với giờ học tiếng Anh! Cô Chim Xanh tin con sẽ học rất giỏi!"
];

const GREETINGS_GUEST = [
    "Chào bé yêu, cô Chim Xanh rất vui được cùng con luyện tiếng Anh hôm nay!",
    "Chào mừng bé đến với lớp tiếng Anh của cô Chim Xanh! Mình cùng thử sức xem sao nhé!",
    "Cô Chim Xanh chào bé! Cùng khám phá từ vựng mới thật vui nào!",
    "Chào thiên tài nhí! Cô Chim Xanh đang chờ xem con nói tiếng Anh giỏi cỡ nào đây!",
...
function speakCurrentQuestion() {
    const q = activeQuestionsList[currentQIndex];
    if (!q) return;
    // SỬA LỖI: bản lớp 3, field "audio_text" (nạp từ "audio" trong JSON) ở đa số chuyên mục
    // (Odd One Out, Vocabulary, Grammar...) TRÙNG với đáp án đúng — nếu đọc nó trước khi học
    // sinh chọn thì vô tình đọc lộ ngay đáp án. CHỈ nhóm Nghe hiểu thật (ENG_LIS) mới có mục
    // đích sư phạm là "nghe âm/từ rồi chọn đáp án" nên đọc audio_text trước là đúng ý đồ.
    // Các nhóm còn lại: đọc thẳng câu hỏi tiếng Anh (question_text đã là tiếng Anh ở bản lớp 3).
    ...
    if (q.question_text) return speakEnglish(q.question_text);
